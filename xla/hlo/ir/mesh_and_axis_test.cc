...
TEST(MeshAndAxisTest, MaximalMesh) {
  ...
  Mesh mesh_single_axis({2}, {"a"});
  EXPECT_FALSE(mesh_single_axis.IsMaximal());

  EXPECT_EQ(Mesh(5), Mesh(5));
  EXPECT_NE(Mesh(5), Mesh(6));
