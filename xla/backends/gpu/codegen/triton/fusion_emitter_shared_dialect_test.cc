...
ENTRY e {
  ...
  TF_EXPECT_OK(CreateXTileIrAndFileCheck(
      ...
      block_level_parameters,
      R"(
CHECK: %[[RES:.*]] = stablehlo.dot %[[ARG0:.*]], %[[ARG1:.*]], precision = [DEFAULT] : (tensor<32x8xf32>, tensor<8x8xf32>) -> tensor<32x8xf32>
CHECK: %[[ADD_RES:.*]] = stablehlo.add %[[ARG2:.*]], %[[RES]] : tensor<32x8xf32>
)"));
}
