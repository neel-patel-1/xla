...
class AllReduceKernelTest : public ::testing::Test,
 ...
 public:
  ...
  absl::StatusOr<std::vector<Array<T>>> RunKernel(
    ...
    std::vector<uint64_t> param_to_peers_ptrs(kNumPeerParameters * num_ranks);
    for (int i = 0; i < num_ranks; ++i) {
      param_to_peers_ptrs[i] =
          reinterpret_cast<uint64_t>(symmetric_input_buffers[i].opaque());
    }
    for (int i = 0; i < num_ranks; ++i) {
      param_to_peers_ptrs[num_ranks + i] =
          reinterpret_cast<uint64_t>(signal_flags_buffers[i].opaque());
    }
