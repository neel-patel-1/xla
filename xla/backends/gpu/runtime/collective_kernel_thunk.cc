...
absl::Status CollectiveKernelThunk::ExchangeStateMetadata(
  ...
  std::vector<uint64_t> param_to_peers_ptrs(rendezvous_values->size() *
                                            kNumPeerParameters);
  for (int i = 0; i < rendezvous_values->size(); ++i) {
    param_to_peers_ptrs[i] = (reinterpret_cast<uint64_t>(
        rendezvous_values->at(i).buffer_ptr.opaque()));
  }
  for (int i = 0; i < rendezvous_values->size(); ++i) {
    param_to_peers_ptrs[rendezvous_values->size() + i] =
